...
def decode(syndrome_grid, mirror_factor = 3, retry = 4, tao_max = None, c_step = 0.2):
    ...
    Args:
        syndrome_grid (np.array): grid containing the error syndrome, this is a np array of size 2*code_distance. The data-qubits are 0, The measurement qubits are 1 if they have been triggered by an error, 0 otherwise
    ...
    while(not is_x_clean and retry_x>0):
        ...
        x_moves, x_convergence_time, is_x_clean = evolve_system(x_field, x_anyons.copy(), tao_max, tb_bound, 0)
        retry_x = retry_x - 1
    ...
    while(not is_z_clean and retry_z>0):
        ...
        z_moves, z_convergence_time, is_z_clean = evolve_system(z_field, z_anyons.copy(), tao_max, lr_bound, 1)
        retry_z = retry_z - 1
    
    retry = max(retry-retry_z, retry-retry_x)
